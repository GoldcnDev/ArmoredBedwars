...
public class BWCommand implements CommandExecutor {
    ...
    public void registerSubCommand(SubCommand command) {
        ...
        for (String alias : command.getAliases()) {
            if (this.subCommandMap.containsKey(alias.toLowerCase())) {
                throw new RuntimeException("Duplicate alias '" + alias + "' for commands: '" +
                        command.getName() + "' and '" + this.subCommandMap.get(alias.toLowerCase()).getName() + "'");
            }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length == 0) {
            MessageUtils.message(sender, "&cInvalid arguments! Try: /bedwars help");
            return true;
        }
        ...
        if (!this.subCommandMap.containsKey(argument)) {
            MessageUtils.message(sender, "&cInvalid arguments! Try: /bedwars " + this.getSuggestion(sender, argument, "help"));
            return true;
        }
