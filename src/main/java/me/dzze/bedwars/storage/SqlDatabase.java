package me.dzze.bedwars.storage;
...
public class SqlDatabase extends AbstractSqlDatabase {
    ...
    public CompletableFuture<Void> createPlayer(String name, UUID uuid) {
        return CompletableFuture.runAsync(() -> {
            if (!this.exists(uuid).join()) {
                try (final PreparedStatement statement = this.getConnection().prepareStatement("INSERT IGNORE INTO points (NAME,UUID) VALUES (?,?)")) {
    ...
    public CompletableFuture<Boolean> exists(UUID uuid) {
        return this.makeFuture(() -> {
            try (final PreparedStatement statement = this.getConnection().prepareStatement("SELECT * FROM points WHERE UUID=?")) {
    ...
    public CompletableFuture<Void> setPoints(UUID uuid, int points) {
        return CompletableFuture.runAsync(() -> {
            try (final PreparedStatement statement = this.getConnection().prepareStatement("UPDATE points SET POINTS=? WHERE UUID=?")) {
    ...
    public CompletableFuture<Integer> getPoints(UUID uuid) {
        return this.makeFuture(() -> {
            try (final PreparedStatement statement = this.getConnection().prepareStatement("SELECT POINTS FROM points WHERE UUID=?")) {
                statement.setString(1, uuid.toString());
                try (final ResultSet result = statement.executeQuery()) {
                    if (result.next()) {
    ...
    public CompletableFuture<Void> emptyTable() {
        return CompletableFuture.runAsync(() -> {
            try (final PreparedStatement statement = this.getConnection().prepareStatement("TRUNCATE points")) {
                statement.executeUpdate();
            } catch (SQLException e) {
                e.printStackTrace();
            }
    ...
    public CompletableFuture<Void> removeTable(UUID uuid) {
        return CompletableFuture.runAsync(() -> {
            try (final PreparedStatement statement = this.getConnection().prepareStatement("DELETE FROM points WHERE UUID=?")) {
                statement.setString(1, uuid.toString());
                statement.executeUpdate();
            } catch (SQLException e) {
                e.printStackTrace();
