...
public class SetDiamondCommand implements SubCommand {
    @Override
    public void execute(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 1) {
            MessageUtils.message(sender, "&cInvalid arguments! Try: /" + label + " dia <1-4>");
            return;
        }
        ...
        try {
            ...
            if (number < 1 || number > 4) {
                ...
                configuration.set(player.getLocation().getWorld().getName() + "." + "Dia." + numberStr, player.getLocation());
                Main.getInstance().saveConfig();

                MessageUtils.message(sender, "&6BEDWARS &8| &aSet diamond gen " + numberStr + " to your location.");
            }
